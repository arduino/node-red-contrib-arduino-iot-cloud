...
const accessTokenUri =
  process.env.NODE_RED_ACCESS_TOKEN_URI ||
  "https://api2.arduino.cc/iot/v1/clients/token";
const accessTokenAudience =
  process.env.NODE_RED_ACCESS_TOKEN_AUDIENCE || "https://api2.arduino.cc/iot";
const arduinoIotCloudHost =
  process.env.NODE_RED_MQTT_HOST || "wss.iot.arduino.cc";
      ...
      RED.nodes.eachNode((n) => {
        if (n.type === "property in") {
          ...
          node.status({
            fill: "red",
            shape: "dot",
            text: "arduino-iot-cloud.status.offline",
          });
        }
      });
    },
  ...
  try {
    ...
    if (user === -1) {
      ...
      if (tokenInfo !== undefined) {
        const ArduinoIotCloudOptions = getMqttOptions(
          connectionConfig.credentials.clientid,
          tokenInfo.token,
          RED,
        );
        const timeout = setTimeout(() => {
          updateToken(connectionConfig);
  ...
  try {
    ...
    if (user === -1) {
      ...
      if (connections[user].clientHttp !== null) {
        ...
        clientHttp = new ArduinoClientHttp.ArduinoClientHttp(
          connections[user].token,
        );
  ...
  try {
    ...
    if (user !== -1) {
      ...
      if (tokenInfo !== undefined) {
        ...
        connections[user].timeoutUpdateToken = setTimeout(() => {
          updateToken(connectionConfig);
        }, tokenInfo.expires_in * 1000);
      } else {
...
async function deleteClientMqtt(clientId, thing, propertyName, nodeId) {
  ...
  if (user !== -1) {
    if (connections[user].clientMqtt !== null) {
      var ret = await connections[user].clientMqtt.removePropertyValueCallback(
        thing,
        propertyName,
        nodeId,
      );
