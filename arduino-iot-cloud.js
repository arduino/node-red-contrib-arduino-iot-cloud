...
module.exports = function (RED) {
    ...
    const realConstructor = async (config) => {
      ...
      if (
        ...
        try {
          ...
          this.arduinoClient = await connectionManager.getClientMqtt(
            connectionConfig,
            RED,
          );
          if (this.arduinoClient && this.arduinoClient.connection.connected) {
            await this.arduinoClient.onPropertyValue(
              this.thing,
              this.propertyVariableName,
              (msg) => {
                this.send({
            ...
            this.status({
              fill: "red",
              shape: "ring",
              text: "arduino-iot-cloud.status.connection-error",
            });
          }
      ...
      if (
        ...
        try {
          if (config.thing !== "" && config.property !== "") {
            this.arduinoRestClient =
              await connectionManager.getClientHttp(connectionConfig);
            if (this.arduinoRestClient) {
              ...
              this.on("input", async function (msg) {
                try {
                  await this.arduinoRestClient.setProperty(
                    this.thing,
                    this.propertyId,
                    msg.payload,
                    opts,
                    this.sendasdevice ? this.device : undefined,
                  );
                  var s;
                  ...
                  if (
                    ...
                    console.log(
                      "statusCode: " +
                        ...
                        "\n" +
                        "statusMessage: " +
                        err.response.res.statusMessage +
                        "\n" +
                        "text: " +
                  ...
                  this.status({
                    fill: "red",
                    shape: "dot",
                    text: "arduino-iot-cloud.status.error-setting-value",
                  });
                }
              });
              this.on("close", function (done) {
                connectionManager
                  .deleteClientHttp(connectionConfig.credentials.clientid)
                  .then(() => {
                    done();
                  });
              });
              ...
              node.on("input", async function () {
                try {
                  ...
                  if (
                    ...
                    const start = now
                      .subtract(count * this.timeWindowUnit, "second")
                      .format();

                    const result = await this.arduinoRestClient.getSeries(
                      this.thing,
                      this.propertyId,
                      start,
                      end,
                  ...
                  this.status({
                    fill: "red",
                    shape: "dot",
                    text: "arduino-iot-cloud.status.error-getting-value",
                  });
                }
              });
          ...
          if (this.arduinoRestClient) {
            ...
            if (config.thing !== "" && config.property !== "") {
              ...
              if (
                ...
                pollTime !== "" &&
                pollTime !== undefined &&
                Number.isInteger(parseInt(pollTime)) &&
                parseInt(pollTime) !== 0
              ) {
                ...
                this.on("close", function (done) {
                  ...
                  if (this.pollTimeoutPoll) clearTimeout(this.pollTimeoutPoll);
                });
              }
            }
          } else {
  ...
  ArduinoIotInputPoll.prototype = {
    poll: async function (connectionConfig, pollTime, organization) {
      try {
        ...
        const property = await this.arduinoRestClient.getProperty(
          this.thing,
          this.propertyId,
          opts,
        );
        ...
        this.pollTimeoutPoll = setTimeout(() => {
          this.poll(connectionConfig, pollTime, organization);
        }, pollTime * 1000);
      } catch (err) {
        if (err.response && err.response.res && err.response.request) {
          console.log(
  ...
  async function getThingsOrProperties(req, res, thingsOrProperties) {
    ...
    try {
      ...
      if (thingsOrProperties === "things") {
        ...
        return res.send(
          JSON.stringify(await arduinoRestClient.getThings(opts)),
        );
        ...
        return res.send(
          JSON.stringify(await arduinoRestClient.getProperties(thing_id, opts)),
        );
        ...
        return res.send(
          JSON.stringify(await arduinoRestClient.getThing(thing_id, opts)),
        );
      } else {
  ...
  RED.httpAdmin.get(
    "/things",
    RED.auth.needsPermission("Property-in.read"),
